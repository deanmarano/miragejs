...
class DbCollection {
  ...
  insert(data) {
    if (!Array.isArray(data)) {
      ...
      const result = map(data, attrs => this._insertRecord(attrs));
...
class Db {
  ...
  createCollection(name, initialData) {
    if (!this[name]) {
      ...
      newCollection._async = this._container?.async ?? this._async ?? false;
...
class DeleteShorthandRouteHandler extends BaseShorthandRouteHandler {
  ...
  */
  handleArrayShorthand(request, modelClasses) {
    ...
    let childTypes = modelClasses.slice(1).map(modelClass => this._container.inflector.pluralize(modelClass.camelizedModelName));
...
class Model {
  ...
  */
  ...
  async _saveAsync(collection) {
    ...
    let isNewRecord = await this._isNewAsync();
    if (isNewRecord) {
      // Update the attrs with the db response
  ...
  async _isNewAsync() {
    ...
    if (hasId) {
      ...
      let record = await this._schema.db[collectionName].find(this.attrs.id);
      if (record) {
        hasDbRecord = true;
      }
    }
...
class Schema {
   ...
   */
  get _isAsync() {
    return this._container?.async ?? false;
  }
  ...
  find(type, ids) {
    ...
    if (this._isAsync) {
      return records.then(r => {
        if (Array.isArray(ids)) {
          assert(r.length === ids.length, `Couldn't find all ${this._container.inflector.pluralize(type)} with ids: (${ids.join(",")}) (found ${r.length} results, but was looking for ${ids.length})`);
        }
...
const Pretender = typeof PretenderModule__namespace.default === 'function' && PretenderModule__namespace.default || typeof PretenderModule__namespace.default?.default === 'function' && PretenderModule__namespace.default.default || typeof PretenderModule__namespace === 'function' && PretenderModule__namespace || PretenderModule__namespace.default || PretenderModule__namespace;
...
/**
 @hide
 */
const defaultPassthroughs = ["http://localhost:0/chromecheckurl",
// mobile chrome
...
exports.trait = trait;
//# sourceMappingURL=mirage.cjs.map
