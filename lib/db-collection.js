...
import { maybeAsync, maybeAsyncApply } from "./utils/async-helpers.js";
...
class DbCollection {
  ...
  */
  ...
  _findRecord(id) {
    ...
    // Check if id is a Promise - this happens when code forgets to await in async mode
    if (id instanceof Promise) {
      throw new Error(
        "Mirage: Received a Promise as an ID. In async mode, you must await all " +
        "database operations and relationship getters. For example:\n" +
        "  const user = await server.schema.users.find(1);\n" +
        "  const post = await user.posts; // must await relationship access\n" +
        "Check your code for missing 'await' keywords."
      );
    }
  ...
  _findRecordsWhere(query) {
    ...
    if (isAsync) {
      ...
      return Promise.all(records.map(record => queryFunction(record)))
        .then(results => records.filter((_, i) => results[i]));
    }
