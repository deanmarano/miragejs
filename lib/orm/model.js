...
class Model {
  ...
  */
  ...
  alreadyAssociatedWith(model, association) {
    ...
    // In async mode, relationship getters return Promises
    if (this._schema._isAsync && associatedModelOrCollection && typeof associatedModelOrCollection.then === "function") {
      // This is a Promise - we can't synchronously check, so return false
      // The async version should be used in async contexts
      return false;
    }
  ...
  async alreadyAssociatedWithAsync(model, association) {
    let associatedModelOrCollection = this[association.name];
    // Await if it's a Promise (async mode relationship getter)
    if (associatedModelOrCollection && typeof associatedModelOrCollection.then === "function") {
      associatedModelOrCollection = await associatedModelOrCollection;
    }
    if (associatedModelOrCollection && model) {
      if (associatedModelOrCollection instanceof Model) {
        if (associatedModelOrCollection.isSaved() && model.isSaved()) {
  ...
  associate(model, association) {
    ...
    let rel = this[name];
    // In async mode, relationship getters return Promises - can't handle synchronously
    if (this._schema._isAsync && rel && typeof rel.then === "function") {
      // This is a Promise - we can't synchronously associate, so skip
      // The async version should be used in async contexts
      return;
    }

    if (association instanceof HasMany) {
      if (!rel.includes(model)) {
        rel.add(model);
      }
    } else {
      this[name] = model;
  ...
  async associateAsync(model, association) {
    if (await this.alreadyAssociatedWithAsync(model, association)) {
      return;
    }
  ...
  */
  ...
  _validateForeignKeyExistsInDatabase(foreignKeyName, foreignKeys) {
    // Skip validation in async mode since find() returns Promises
    // and this validation runs in the constructor which can't be async
    if (this._schema._isAsync) {
      return;
    }
  ...
  async _saveNewAssociatesAsync(association) {
    ...
    if (tempAssociate !== undefined) {
      ...
      if (tempAssociate instanceof Collection) {
        ...
        // Clearing the association
        if (tempAssociate === null) {
          await this._updateInDbAsync({ [fk]: null });

          // Self-referential
        } else if (this.equals(tempAssociate)) {
          await this._updateInDbAsync({ [fk]: this.id });
          ...
          await this._updateInDbAsync({ [fk]: fkValue });
        }
      }
  ...
  */
  ...
  async _updateInDbAsync(attrs) {
    this.attrs = await this._schema.db[
      this._schema.toInternalCollectionName(this.modelName)
    ].update(this.attrs.id, attrs);
  }
